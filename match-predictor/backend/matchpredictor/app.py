...
from matchpredictor.predictors.decision_tree_predictor import train_tree_predictor
from matchpredictor.predictors.boosted_tree_predictor import train_boosted_predictor
from matchpredictor.predictors.adaboosted_tree_predictor import train_adaboosted_predictor
from matchpredictor.predictors.bagged_tree_predictor import train_bagged_predictor
from matchpredictor.predictors.random_forest_predictor import train_rf_predictor
...
def build_model_provider(training_data: List[Result]) -> ModelProvider:
    return ModelProvider([
        Model("Alphabetic", AlphabetPredictor()),
        Model("Home", HomePredictor()),
        Model("Points", train_results_predictor(training_data)),
        Model("Decision Tree", train_tree_predictor(training_data)),
        Model("Boosted Tree", train_boosted_predictor(training_data)),
        Model("AdaBoosted Tree", train_adaboosted_predictor(training_data)),
        Model("Bagged Tree", train_bagged_predictor(training_data)),
        Model("Random Forest", train_rf_predictor(training_data)),
        ...
        Model("Kernel Ridge Regression", train_kernel_predictor(training_data))
    ])
