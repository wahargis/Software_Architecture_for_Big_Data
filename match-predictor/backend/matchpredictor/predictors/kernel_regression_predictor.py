...
class KernelRidgeRegressionPredictor(Predictor):
    def __init__(self, model: KernelRidge, team_encoding: OneHotEncoder) -> None:
        self.model = model
...
def build_model(results: List[Result]) -> Tuple[KernelRidge, OneHotEncoder]:
...
def train_kernel_predictor(results: List[Result]) -> Predictor:
    model, team_encoding = build_model(results)

    return KernelRidgeRegressionPredictor(model, team_encoding)
