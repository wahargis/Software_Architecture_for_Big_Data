...
from sklearn.ensemble import HistGradientBoostingClassifier, BaggingClassifier  # type: ignore
...
class BaggedTreePredictor(Predictor):
...
def build_model(results: List[Result]) -> Tuple[HistGradientBoostingClassifier, OneHotEncoder]:
    ...
    model = BaggingClassifier(HistGradientBoostingClassifier(learning_rate=0.1),max_samples=0.5, max_features=0.5)
    model.fit(x, y)

    return model, team_encoding


def train_bagged_predictor(results: List[Result]) -> Predictor:
    model, team_encoding = build_model(results)

    return BaggedTreePredictor(model, team_encoding)
