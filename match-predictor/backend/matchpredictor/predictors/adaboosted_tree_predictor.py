...
class AdaBoostedTreePredictor(Predictor):
    def __init__(self, model: AdaBoostClassifier, team_encoding: OneHotEncoder) -> None:
        self.model = model
...
def build_model(results: List[Result]) -> Tuple[DecisionTreeClassifier, OneHotEncoder]:
...
def train_adaboosted_predictor(results: List[Result]) -> Predictor:
    model, team_encoding = build_model(results)

    return AdaBoostedTreePredictor(model, team_encoding)
